...
const events = [
  ...
  ['Monday · 21 September 2026', 'Haldi', 'Turmeric Blessings', '11:00 AM', 'At Our residence', 'https://www.google.com/maps?q=32.311924,75.607025', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR6JPb9Hc1Sm_4cS_8-llAkutHJUL5WPjiejEakYJHNbFax418w6MhUj35cKV-iWbo&s=10&ec=121902066'], 
...
const venues = [
  ['House 74', 'Near Sagar Chemist Sujanpur', 'https://www.google.com/maps?q=32.311924,75.607025'],
 ...
 const startMusic = async () => { const track = audio.current; if (!track) return false; track.volume = 0.38; track.muted = false; if (track.readyState === 0) track.load(); try { await track.play(); setMusicOn(true); return true; } catch { setMusicOn(false); return false; } };
  ...
  return <main>
    ...
    <div className={opened ? 'site revealed' : 'site'}>
      ...
      <section id="home" className="hero"><Petals /><p className="eyebrow">With the blessings of our families</p><div className="hero-om">ॐ</div><h1>Ish <i>&amp;</i> Dr. Arshika</h1><span className="hero-divider" /><p className="forever">are beginning their forever</p><div className="date">21 <span>·</span> September <span>·</span> 2026</div><div id="countdown" className="hero-countdown"><Countdown /></div></section>
      <section className="family"><p className="eyebrow">A union of hearts</p><h2>Two families, one<br /><i>beautiful story</i></h2><p>Ish, beloved son of Mrs. Renu &amp; Late Shri Ashwani Mahajan, with Dr. Arshika, beloved daughter of Mrs. Jyoti &amp; Mr. Sanjeev Sharma, invite you to share in their joy.</p></section>
      <section id="events" className="events editorial-events"><header><p className="eyebrow">The celebrations</p><h2>Chapters of <i>celebration</i></h2><p>Every ritual, a memory<br />every moment, a blessing</p></header><div className="editorial-grid"><div className="event-list">{events.map((event, i) => <article className={activeEvent === i ? 'event active' : 'event'} key={event[1]} onMouseEnter={() => setActiveEvent(i)} onFocus={() => setActiveEvent(i)} onClick={() => setActiveEvent(i)} tabIndex="0"><div className="event-copy"><h3>{event[1]}</h3><i>{event[2]}</i></div><div className="event-details"><p className="event-date">{event[0]}</p><p className="event-time">{event[3]}</p></div><p className="event-venue">{event[4]}</p><img className="event-mobile-image" src={event[6]} alt="" loading="lazy" /><a className="event-map" href={event[5]} target="_blank" rel="noreferrer" aria-label={`View ${event[1]} venue in Google Maps`}>View <span>→</span></a></article>)}</div><aside className="event-showcase"><img src={events[activeEvent][6]} alt={`${events[activeEvent][1]} celebration`} /></aside></div></section>
      ...
      <footer id="rsvp"><div className="footer-ring ring-a" /><div className="footer-ring ring-b" /><p className="eyebrow">With love and warm wishes</p><h2>Your presence is<br /><i>our present</i></h2><div className="contacts"><a href="tel:+919988388988"> Ish +91 99883 88988  </a><a href="tel:+918284824808">Ankit +91 82848 24808</a></div></footer>
    </div>
